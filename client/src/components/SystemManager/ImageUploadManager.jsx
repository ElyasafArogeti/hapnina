...
useEffect(() => {
  const fetchImages = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:3001/getUploadedImages',{
        headers: {
  ...
  const handleDeleteImage = async (public_id) => {
    try {
      ...
      const response = await axios.delete(`http://localhost:3001/deleteImage/${encodedPublicId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        }
      });
  ...
  const handleUpdateImage = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:3001/updateImage/${editingImage.public_id}`, { name: imageName });
